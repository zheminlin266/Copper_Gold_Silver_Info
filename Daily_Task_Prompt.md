...
8. X 只收录原作者帖子。按日期保存原始候选到 `x_outputs/REPORT_DATE_x_raw_materials.txt`，不得覆盖历史文件。完整检索后确实没有合格帖子时可以发布空数组；若登录或通道失败，将 `part2_searched` 设为 `false`、`part2_channel` 设为 `failed` 并记录原因，继续完成 Part 1 和 Part 3，但最终不得把失败当作零结果发布，也不得用搜索摘要或截图替代原帖。
