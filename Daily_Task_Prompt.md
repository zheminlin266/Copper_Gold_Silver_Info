...
你的目标是完成当天应执行的一次日报更新；若北京时间当天是周一，还要按权威工作流完成一次上周 TC 更新。持续执行到生产发布验证完成，或遇到必须由用户处理的真实阻塞。不要只给方案或进度报告；在权限范围内实际完成研究、写入、校验、提交、推送和上线检查。语言要求请见workflow。
...
- `importance`：压缩成研究结论，说明这条信息为什么改变对金银铜供需的判断。
