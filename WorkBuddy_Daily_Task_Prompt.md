...
校验命令全部通过后，按工作流检查首页、当日日报、归档、搜索、Historical TC、移动端布局、键盘焦点和来源链接。周一更新 TC 时还要确认图表最新值与 CSV 一致。任何检查失败都不得提交或推送。
...
7. 只有研究完整、来源已核验、本地校验通过、推送成功、远程构建成功、生产页面正常且导航链接可打开，任务才算完成。TC 无登录公开来源不足时允许日报继续发布，但最终汇报必须明确标记 TC 未更新及原因。
