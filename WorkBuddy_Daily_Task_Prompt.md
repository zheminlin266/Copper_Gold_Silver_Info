...
9. 网站内容只新增 `data/REPORT_DATE.json`。其中 `summary` 不超过 300 个字符，只按金属概括当日供需方向，不罗列检索过程、渠道状态或收录数量。只有工作流明确允许的按日期原始材料和经核验的新来源登记可以追加。不要生成 HTML、图片、AI 模块或搜索索引，不要手改首页、日报组件、归档代码和旧报告。
