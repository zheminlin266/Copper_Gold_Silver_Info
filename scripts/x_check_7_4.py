"""
Check X posts for 2026-07-04 Beijing window using Playwright + storage_state.
...
OUTPUT_FILE = PROJECT_ROOT / "x_outputs" / "2026-07-04_x_raw_materials.txt"

BEIJING_TZ = timezone(timedelta(hours=8))
DAY = "2026-07-04"
WINDOW_START = datetime(2026, 7, 4, 0, 0, 0, tzinfo=BEIJING_TZ)
WINDOW_END = datetime(2026, 7, 4, 23, 59, 59, tzinfo=BEIJING_TZ)
...
async def check_handle(page, handle):
    """Search a handle for posts within the 7-4 Beijing window."""
    results = []
...
async def main():
    ...
    if not all_results:
        output.append("- No qualifying posts found in the Beijing window. All searched handles either had no posts, or their posts fell outside the 2026-07-04 00:00-23:59 Asia/Shanghai window.")
    else:
    ...
    if not all_results:
        output.append("- Search worked and checked all 34 seed handles + 18 official accounts.")
    ...
    print(f"Total candidates in 7-4 window: {len(all_results)}")
