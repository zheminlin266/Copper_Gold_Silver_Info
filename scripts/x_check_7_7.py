"""
Check X posts for 2026-07-07 Beijing window using Playwright + storage_state.
"""
import asyncio, sys
...
OUTPUT_FILE = PROJECT_ROOT / "x_outputs" / "2026-07-07_x_raw_materials.txt"

BEIJING_TZ = timezone(timedelta(hours=8))
DAY = "2026-07-07"
WINDOW_START = datetime(2026, 7, 7, 0, 0, 0, tzinfo=BEIJING_TZ)
WINDOW_END = datetime(2026, 7, 7, 23, 59, 59, tzinfo=BEIJING_TZ)
...
async def main():
    if not STORAGE_STATE.exists():
        print("ERROR: x_auth.json not found at", STORAGE_STATE)
        print("Please run scripts/setup_x_login.py first to create login state.")
        sys.exit(1)
    ...
    if not all_results:
        output.append("- Search completed all handles. No qualifying posts for this Beijing-day window.")
    else:
