"""
Check X posts for 2026-07-10 Beijing window using Playwright + storage_state.
...
OUTPUT_FILE = PROJECT_ROOT / "x_outputs" / "2026-07-10_x_raw_materials.txt"

BEIJING_TZ = timezone(timedelta(hours=8))
DAY = "2026-07-10"
WINDOW_START = datetime(2026, 7, 10, 0, 0, 0, tzinfo=BEIJING_TZ)
WINDOW_END = datetime(2026, 7, 10, 23, 59, 59, tzinfo=BEIJING_TZ)
...
async def check_handle(page, handle):
    """Search a handle for posts within the 7-10 Beijing window."""
    results = []
...
async def main():
    ...
    if not all_results:
        ...
        output.append('- Final result: "searched but no qualifying posts."')
    else:
    ...
    print(f"Total candidates in 7-10 window: {len(all_results)}")
