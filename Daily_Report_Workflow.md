...
- 某一部分采集失败：把对应 `*_searched` 写为 `false` 并记录失败；不得把失败写成零结果，最终不得发布。X 登录失效时仍继续其他部分以保留研究成果，但不以搜索摘要代替原帖，也不发布不完整日报。
...
- [ ] 日期和三个窗口计算正确。
- [ ] 三部分都完成检索，三个 `*_searched` 均为 `true`；空结果也有检索范围记录，采集失败没有冒充零结果。
