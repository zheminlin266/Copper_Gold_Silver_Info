...
本文是 生成“金银铜供需信息”日报及执行每周 TC 更新的唯一执行规范。网站代码只负责展示；每日任务新增一份日报 JSON，周一任务还可以按第 9A 节追加一条 TC CSV 数据，不生成 HTML，不手工更新首页。页面用中文，如原文是英文请翻译成地道中文，在专有名词、术语时保留英文原词。
...
`importance` 不是标题改写，也不是“这是一个重要信号”的标签。它必须给出研究结论：这条信息相对于已有信息改变了什么，以及为什么值得关注。

每条新信号的 `importance` 必须满足：
...
`interpretation` 负责展开事实如何传导、有哪些假设和限制；`importance` 负责压缩成面向读者的研究结论，不能退化为泛化标签。对 X 帖子或分析观点可以降低证据等级，但仍须说明新增事实、可能影响和验证条件。
...
- 每条新信号都包含有实质内容的 `importance`；新闻还包含 `source`、`title`、`publish_time`、`language`、`excerpt` 和 `interpretation`。
...
- 新日报每条信号的 `importance` 都通过 2–4 句、80–300 字、具体锚点和非泛化结论检查；历史日报按历史兼容规则保留，不因本次规则回填。
...
- [ ] 事实、数字、口径和研究判断明确分开。
- [ ] 每条新信号的重要性判断都说明规模、机制、时间、对比或不确定性中的至少三项，并通过内容校验。
- [ ] 重复事件已排除并记录。
